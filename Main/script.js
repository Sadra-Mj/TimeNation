(() => {
  ...
  // Supports both IDs: #citizenshipForm OR #applyForm
  // --------------------------------------------------
  const form = $("#citizenshipForm") || $("#applyForm");
  const statusEl = $("#formStatus");

  // Supports both button IDs: #prefillExample OR #prefillBtn
  const prefillBtn = $("#prefillExample") || $("#prefillBtn");

  if (prefillBtn && form) {
    prefillBtn.addEventListener("click", () => {
      // Fill only if those fields exist
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      // Optional agreement checkbox validation if present
      const agree =
      ...
      if (agree && !agree.checked) {
        if (statusEl) statusEl.textContent = "❌ Please confirm the privacy policy before submitting.";
        return;
      }
      ...
      try {
        ...
        const res = await fetch("/api/citizenship/apply", {
          method: "POST",
          body: formData,
      });
    ...
    if (statusEl) {
      statusEl.textContent = `✅ Submitted successfully. Application ID: ${data.applicationId}`;
    }
